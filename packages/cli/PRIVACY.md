...
ログファイルの各JSONL行を解析します。集計・送信・保存・表示に利用するのは、日付、
モデル名、トークン数などの利用量フィールドのみです。プロンプト本文、出力本文、
ソースコード、ファイルパス、環境変数、認証情報は送信・保存・表示しません。
...
- `sourceId`(ランダムUUID): ログフォルダに識別用IDファイル(`.shibaita-source-id`、
  ...
  情報は一切含まれません。
- `os`(OS種別。`macos` / `windows` / `linux` / `other` の4値のみ): Node.jsの
  `process.platform` から機械的にマッピングした値です。ホスト名・マシン名・
  OSバージョン等は一切送信されません。マイページの「端末別内訳」表示にのみ利用します。
- レート制限に達した回数(日別の件数のみ): ログ行の `error.rateLimits` フィールドの
  **有無だけ**を検出し、日付ごとの検出件数(`date` / `count`)のみを送信します。
  `rateLimits` フィールドの中身(リセット時刻等)は一切読まず、保持も送信もしません。
  対象期間内に検出がなければこのフィールド自体を送信しません。現時点では画面表示は
  行わず、保存のみです。
...
- ホスト名、OSユーザー名、端末識別子
- OSバージョン・アーキテクチャ等の詳細情報(送信するのは `macos`/`windows`/`linux`/`other` の4値のみ)
- MCPサーバー名等の環境構成情報
