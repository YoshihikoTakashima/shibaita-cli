...
マシン名は送りません)を送信します。加えて、レート制限に達した回数(日別の件数のみ。
中身の値は読まず保存しません)も対象期間内にあれば送信します。

## FAQ

**Q. 複数アカウントで同じPCは使えますか?**

A. 使えません。1端末(=1つのログフォルダ)は常に1アカウントとひも付きます
(`.shibaita-source-id` のUUID単位)。別のアカウントで `pair`/`login` を試みると
「このPC(ログフォルダ)は既に別のアカウントに連携されています」というエラーになります。
アカウントを切り替えたい場合は、旧アカウントでログインして [設定画面](https://shibaita.ai/settings)
から該当端末の連携を解除するか、旧アカウントを退会してください(退会・解除するとその
端末分の記録は削除されます)。
